...
export const getAllAssignments = async (req, res) => {
    ...
    try {
        ...
        const fetchCourseAssignments = async (course) => {
            ...
            return courseWork.map((ga) => {
                ...
                return {
                    ...
                    type: ga.materials && ga.materials[0]?.driveFile?.driveFile?.title
                        ? getFileType(extname(ga.materials[0].driveFile.driveFile.title)
                            .toLowerCase()
                            .slice(1))
                        : null,
                    thumbnail: (ga.materials &&
        ...
        const allAssignments = await Promise.all(courses.map((course) => fetchCourseAssignments(course)));
...
export const getYoutubeVideos = async (req, res) => {
    ...
    try {
        const response = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&q=${encodeURIComponent(keywords)}&type=video&maxResults=4&key=${YOUTUBE_API_KEY}`);
        if (!response.ok) {
